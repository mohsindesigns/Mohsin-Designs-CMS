...
export default function BlogAdminPage() {
  ...
  useEffect(() => {
    // First try fetching page from /api/admin/pages for slug 'blogs' or 'blog'
    fetch("/api/admin/pages")
      .then(res => res.json())
      .then(json => {
        const pages = json.pages || json || [];
        const blogPage = pages.find((p: any) => p.slug === "blogs" || p.slug === "/blogs" || p.slug === "blog" || p.slug === "/blog");
        if (blogPage) {
          setPageId(blogPage._id || blogPage.id || "");
          setData(blogPage.content || {});
        } else {
    ...
    <div className="max-w-6xl mx-auto pb-20 space-y-4">
      ...
      <div className="flex items-center justify-between bg-white border border-[#c3c4c7] px-4 py-3 rounded-[3px] shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-base font-bold text-[#1d2327]">Blogs Index Page Editor</h1>
        </div>
      ...
      <div className="bg-white border border-[#c3c4c7] rounded-[3px] shadow-sm p-5 sm:p-6">
        <BlogEditor pageId={pageId} data={data} setData={setData} />
      </div>
    </div>
  );
}

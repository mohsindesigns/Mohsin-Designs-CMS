...
export default function BlogAdminPage() {
  ...
  useEffect(() => {
    // First try fetching page from /api/admin/pages for slug 'blog'
    fetch("/api/admin/pages")
      .then(res => res.json())
      .then(json => {
        const pages = json.pages || json || [];
        const blogPage = pages.find((p: any) => p.slug === "blog" || p.slug === "/blog");
        if (blogPage) {
    ...
    <div className="max-w-6xl mx-auto pb-20 space-y-4">
      ...
      <div className="flex items-center justify-between bg-white border border-[#c3c4c7] px-4 py-3 rounded-[3px] shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-base font-bold text-[#1d2327]">Blog Index Page Editor</h1>
        </div>
